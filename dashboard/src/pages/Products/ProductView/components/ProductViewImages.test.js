...
describe('Pruebas en el componente <ProductViewImages />', ()=>{
    ...
    let values = {
        ...
        puntos:15250,
        gallery:["https://tienda.havanna.com.ar/wp-content/uploads/2020/10/alfajor70caco.jpg","https://tienda.havanna.com.ar/wp-content/uploads/2020/10/alfajor70caco.jpg"],
        mostwanted:false,
        suggested:false,
        stock:1,
    ...
    describe('Se renderizan las imagenes actuales', () => { 
        ...
        test('Se renderiza el label Imagenes actuales si hay imagenes', ()=>{
            if(values.gallery.length !== 0){
                ...
                const label = screen.getByText("No hay imagenes actuales");
                expect(label).toBeInTheDocument();
            }
        });

        test('Se renderizan las imagenes actuales si hay', ()=>{
            if(values.gallery.length !== 0){
    ...
    describe('Se puede agregar y quitar una imagen', () => { 
        ...
        test('Se puede agregar una imagen', () => { 
            const input = screen.getByLabelText("Nueva imagen", {selector: "input"});
            const oldGalleryLenght = values.gallery.length;

            userEvent.clear(input);
            userEvent.type(input, "NUEVA IMAGEN");
            userEvent.tab();

            expect(values.gallery).toHaveLength(oldGalleryLenght + 1);
